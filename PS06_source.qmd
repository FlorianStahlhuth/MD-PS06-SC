...
* Looking at the scatter plot, there appears to be a strong linear relationship between `hs` and `income`.
:::
...
```{r}
mod_full <- lm(income ~ hs * urbanization, hate_crimes)
get_regression_table(mod_full) |> 
  kable()
```

* The regression line for high `urbanization` has the larger intercept of \$`r format(coef(mod_full)[1], scientific = FALSE)` in comparison to \$`r format(coef(mod_full)[1] + coef(mod_full)[3], scientific = FALSE)`  for low `urbanization`.
:::
...
* Yes, the slope looks fairly similar for the two levels of urbanization.
:::
...
* Since the slopes look fairly similar for both regression lines, a less complex "parallel slopes" model is best in this case. It captures the essential relationship between `income` and the predictors `hs` and `urbanization` without unnecessary complication.
...
ggplot(hate_crimes, aes(x = urbanization, y = income)) +
  ...
  labs(x = "Level of urbanization", 
       y = "Median household income in USD",
       title = "Median household income based on level of urbanization")
...
```{r}
ggplot(hate_crimes, aes(x = hs, y = income, colour = urbanization)) + 
  geom_point() + 
  geom_parallel_slopes(se = FALSE) +
  theme_bw() + 
  labs(x = "Percentage of adults with a high-school degree",
       y = "Median household income in USD", 
       title = "Median household income based on level of urbanization and percentage of adults \n with a high school degree")
...
* No, the intercept is not the same for states with a "low" or "high" level of urbanization. However, the slopes in this case are the same.
:::
...
```{r}
slope_high_urb <- coef(med_income_model)[2]
intercept_high_urb <- coef(med_income_model)[1]
slope_low_urb <- coef(med_income_model)[2]
intercept_low_urb <- coef(med_income_model)[1] + coef(med_income_model)[3]
```

* The slope for the regression line of the states with a "high" level of urbanization is `r dollar(slope_high_urb)`.

* The intercept for the regression line of the states with a "high" level of urbanization is `r dollar(intercept_high_urb)`.
:::
...
* The slope for the regression line of the states with a "low" level of urbanization is `r dollar(slope_low_urb)`.

* The intercept for the regression line of the states with a "low" level of urbanization is `r dollar(intercept_low_urb)`.
:::
...
* Based on the regression table output, median household income is on average `r dollar(abs(coef(med_income_model)[3]))` higher in states with "high" urbanization in comparison to states with "low" urbanization.
:::
...
* For every increase in 1 percentage point of high-school educated adults, the average associated increase in the median household income of a state is `r dollar(coef(med_income_model)[2])`.
:::
...
```{r}
pred_high_urb <- predict(med_income_model, newdata = tibble(urbanization = "high", hs = 85))

```

* The predicted median household income for a state with a high level of urbanization, in which 85% of the share of adults have a high school degree is `r dollar(pred_high_urb)`.
:::
...
```{r}
pred_low_urb <- predict(med_income_model, newdata = tibble(urbanization = "low", hs = 85))

```

* The predicted median household income for a state with a low level of urbanization, in which 85% of the share of adults have a high school degree is `r dollar(pred_low_urb)`.
:::
...
* The predicted median household income for a state with a low level of urbanization, in which 30% of the share of adults have a high school degree is `r dollar(predict(med_income_model, newdata = tibble(urbanization = "low", hs = 30)))`. This value does not make sense as the considered share of adults with a high school degree is significantly outside the range of the data used to fit the model.
:::
...
```{r}
maine_observed <- hate_crimes$income[2]
maine_predicted <- predict(med_income_model, newdata = tibble(hs = hate_crimes$hs[2], urbanization = hate_crimes$urbanization[2]))
maine_residual <- maine_observed - maine_predicted

```

* The observed `income` value for Maine is `r dollar(maine_observed)`. 

* The prediction for Maine according to our model is `r dollar(maine_predicted)`.

* The residual is `r dollar(maine_residual)`.

* This shows our model overestimated the median income for Maine.
:::
...
ggplot(vole_trapping, aes(x = veg, y = voles, color = soil)) + 
  ...
  geom_parallel_slopes(se = FALSE) + 
  theme_bw() + 
  labs(x = "Percentage of site covered with vegetation",
       y = "Number of voles", 
       title = "Number of voles depending on percentage of site covered with vegetation and soil type")
...
* I would recommend to protect sites with a high vegetation cover as there is a positive relationship between percentage of site covered with vegetation and number of voles. Hence, protecting areas with high vegetation cover is expected to benefit a larger number of voles.
:::
...
* In order to support a population of about 30 voles, a conservation organization has to maintain roughly 57% cover of vegetation at the site.

```{r}
predict(voles_mod, newdata = tibble(soil = "dry", veg = 57))
...
```{r}
voles_pred <- predict(voles_mod, newdata = tibble(soil = "moist", veg = 40))


```

* Using the regression equation, the expected vole population a site with moist soil and 40% vegetation cover can support is `r round(voles_pred)` voles.
:::
